...
getInitialDesign <- function(algorithm, input, output) {
    ...
    algorithm$job = future(evaluator=plan("multisession"),lazy = FALSE,{
        sink(file.path(wd,paste0('PSO_',algorithm$id,'.out')),type='output')
        print("Starting psoptim()")
        set.seed(algorithm$seed)
        o = psoptim(par=(min_input(input)+max_input(input))/2,
                  fn=function(x) {
                      ask_Y(id=algorithm$id,x=matrix(x,ncol=algorithm$d))}, 
                  lower=min_input(input), upper=max_input(input), 
                  control=list(maxit=algorithm$maxit,vectorized=T))
        print("psoptim() ended")
        print(o)
        sink(type='output')
