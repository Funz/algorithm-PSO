...
test = function(algorithm_file) {
    results = run.algorithm(algorithm_file, options=list(maxit='20'),fun=list(input=input.f,output=output.f,fun=f))
    if (!isTRUE(test_that("branin_-510_015 min",{expect_equal(as.numeric(results$min),min.f,tolerance = .01)}))) quit(status=1)
}
